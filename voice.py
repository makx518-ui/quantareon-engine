...
# 🎧 27.08 SMART TURN — модель, которая СЛЫШИТ, договорил человек или нет.
# ⚠️ Зачем: наша схема гадала по длине текста и знакам препинания, и на
# длинной мысли с паузами резала человека на куски. Модель слушает саму
# интонацию: договорил — голос падает, взял паузу — голос висит.
# Проверено на его живой речи 27.08: в длинной мысли (29 с, 14 пауз) сказала
# «ждать» 12 раз из 14; на коротких фразах — «отвечать» с уверенностью 0.93+.
# На синтезе Ермила она врёт (0.99 «отвечать» всегда) — синтез не умеет
# оборванной интонации, но живую речь читает верно.
# 8 МБ, работает на процессоре за 40 мс, 23 языка включая русский, BSD-2.
try:
    import sys as _sys
...
class Config:
    ...
    # 🌐 19.09 ИНТЕРНЕТ ВЫКЛЮЧЕН — ВРЕМЕННО, ЕГО РЕШЕНИЕ.
    # ⚠️ ЗАЧЕМ: приманки срабатывали не на том, модель уходила искать там,
    # где искать было нечего, и разговор проваливался в долгую паузу.
    # Пока приманки не выправлены, инструмент модели не даём вовсе —
    # тогда и звать нечего, и падать негде.
    # ВЕРНУТЬ = поставить True. Больше ничего менять не надо: весь код
    # поиска цел и на месте, он просто не зовётся.
    ИНТЕРНЕТ_ВКЛЮЧЁН: bool = False
    ...
    # 🚦 27.08 СКОЛЬКО КУСКОВ ПУСКАТЬ К MICROSOFT РАЗОМ.
    # ⚠️ Замер 27.08: пять кусков разом — 0.6 с, по одному — 21 с. Но десяток
    # разом он не тянет: часть не влезает в семь секунд, идут повторы, и один
    # кусок занимает до 22 секунд. Отсюда провалы в речи на длинном ответе.
    # Четыре — быстро и без отказов. У Яндекса такие ворота стоят давно (шесть).
    EDGE_РАЗОМ: int = 4
    ...
    # ⏱️ 26.08 ДВЕ ПОПЫТКИ ПО ДВЕ СЕКУНДЫ.
    # Вторая нужна не «на всякий случай», а против моргнувшей сети: если
    # Яндекс лёг по-настоящему, повтор упрётся в то же самое, а вот сбой
    # связи он вылечит. Замер: Яндекс отвечает за 0.5–1.4 с, двух хватает
    # с запасом.
    # ⚠️ ПОЧЕМУ ИМЕННО ДВЕ, А НЕ ТРИ СЕКУНДЫ: следом идут три попытки
    # Дмитрия по 7 с. При 2+2 худший случай молчания = 25 с, ровно как
    # было при одной попытке в 3 с. При 3+3 стало бы 28 — это уже потеря.
    # 🔀 27.08 ПОДСТРАХОВКА ДМИТРИЕМ — ВЫКЛЮЧЕНА (его решение).
    # Русская сторона идёт только через Яндекс: не ответил — тишина.
    # Весь код Дмитрия цел, он просто не зовётся. Нужна подстраховка —
    # поставьте True, больше ничего менять не надо.
    ...
    YANDEX_TRIES: int = 2
    # 🚧 26.08 ЗАЩИТА ОТ ПОСТОЯННЫХ ОТКАЗОВ.
    # Грант кончается 23.10. Если его не пополнить, Яндекс начнёт отказывать
    # на КАЖДОЙ фразе, и каждая будет стоить 4 с ожидания перед Дмитрием —
    # разговор станет вязким. Поэтому: три отказа подряд — и на пять минут
    # перестаём к нему ходить, сразу идём к Дмитрию. Через пять минут
    # пробуем снова: вдруг это была временная беда.
    YANDEX_ОТКАЗОВ_ДО_ПАУЗЫ: int = 3
    YANDEX_ПАУЗА_СЕК: float = 300.0
    ...
    # 🔚 27.08 ХВОСТ ТИШИНЫ В КОНЦЕ ОТВЕТА, миллисекунды.
    # ⚠️ Не выдумано, а взято у Дмитрия — на нём всё было выверено месяцами.
    # Замер на одном тексте: у Дмитрия выдох в конце 0.93 с, у Ермила 0.29 с.
    # Недостающие 640 мс дописываем, иначе ответ звучит оборванным и человек
    # думает, что помощник пропал.
    YANDEX_ХВОСТ_МС: int = 640
...
# 🔢 28.08 РАЗБОР СЛОВ ДЛЯ ЦИФРЫ «1».
# ⚠️ Словарь оборотов ниже покрывал четыре случая, а их бесконечно много:
# «на 1 сайте», «в 1 проекте», «за 1 день». Его замечание 28.08: в окне
# осталось «трудимся на 1 сайте».
# Разбор смотрит на СЛЕДУЮЩЕЕ слово — его род и падеж — и ставит «один» в
# нужной форме: сайте (муж., предл.) → одном, волне (жен.) → одной.
# ⚠️ Не поставился — работаем по словарю, как раньше. Движок не падает.
try:
    import pymorphy3 as _pymorphy
    _морф = _pymorphy.MorphAnalyzer()
except Exception:
    _морф = None


def _один_под_слово(следом: str) -> str:
    """Дать «один» в форме, подходящей к слову. Не вышло — вернуть пусто."""
    if not _морф:
        return ""
    try:
        р = _морф.parse(следом)[0]
        род, падеж = р.tag.gender, р.tag.case
        if not род or not падеж:
            return ""
        ф = _морф.parse("один")[0].inflect({род, падеж})
        return ф.word if ф else ""
    except Exception:
        return ""


def _цифра_один_словом(текст: str) -> str:
    """Заменить одинокую «1» перед словом на «один» в нужной форме."""
    if not _морф:
        return текст

    def _замена(м):
        следом = м.group(2)
        слово = _один_под_слово(следом)
        if not слово:
            return м.group(0)
        # ⚠️ Регистр не трогаем: предлог сохраняется как есть в group(1),
        # а «один» внутри предложения всегда строчный. Иначе выходило
        # «В Одном лице» — заглавная попадала не туда.
        return f"{м.group(1)}{слово} {следом}"

    # предлог + «1» + слово: «на 1 сайте», «над 1 проектом», «за 1 день»
    # ⚠️ Время не трогаем: «в 1 час» так и должно писаться цифрой.
    return re.sub(r"\b((?:на|в|во|за|с|со|о|об|от|до|из|у|к|ко|по|при|про|над|"
                  r"под|перед|через|между|около|возле)\s+)"
                  r"1\s+(?!час|часа|часов|году|года|месяце|числа)"
                  r"([А-Яа-яЁё]{3,})\b", _замена, текст, flags=re.IGNORECASE)
...
def _починить_распознанное(текст: str) -> str:
    ...
    # 🔢 и общий случай: любая одинокая «1» перед словом
    т = _цифра_один_словом(т)
    return т
...
# 🗣️ 18.09 РОДНЫЕ ГОЛОСА ДЛЯ СВОЕЙ ПИСЬМЕННОСТИ.
# Эндрю мультиязычный и сам угадывает язык по тексту. На латинице он не
# ошибается: испанский, французский, немецкий, итальянский, португальский
# читает верно. А на японском сорвался — в ответе про сайт много английских
# названий (Astro-Fractal, ConsciousAI, Audit, Coding, API, Labs), он их
# увидел и прочёл ВЕСЬ ответ по-английски. В приветствии, где латиницы почти
# нет, японский звучал правильно — значит дело именно в смеси.
# Для языков со своей письменностью язык теперь задан жёстко, голосом:
# гадать ему не о чем. Английские названия такой голос произнесёт с местным
# выговором — ровно так их и произносят в этих странах.
РОДНЫЕ_ГОЛОСА = {
        ...
        try:
            ...
            if msg_type == "Results":
                ...
                if alternatives:
                    ...
                    # промолчать.
                    # 🌊 27.08 ПОТОК СЛОВ — то, что человек говорит ПРЯМО СЕЙЧАС.
                    # Уходит в окно, чтобы человек видел свои слова на лету.
                    # ⚠️ В БУФЕР НЕ КЛАДЁМ: поток уточняется на ходу («прив» →
                    # «привет» → «привет дружище»), и в реплику попал бы мусор.
                    # Он только для глаз; решает, когда отвечать, по-прежнему
                    # модель, а реплику собирает окончательный текст.
                    if (transcript and not (is_final or speech_final)
                            and confidence > 0.5 and self.on_interim):
                        try:
                    ...
                    elif (transcript and not (is_final or speech_final)
                          and confidence > 0.5):
                        # 🌊 27.08 ПОТОКОВЫЙ ТЕКСТ — слова, пока человек говорит.
                        # ⚠️ В буфер НЕ кладём: он уточняется на ходу («прив» →
                        # «привет» → «привет дружище»), и в реплику попал бы
                        # мусор. Он нужен для другого: по нему видно, что
                        # человек ЕЩЁ ГОВОРИТ — точнее, чем по громкости
                        # микрофона, которую поднимают кашель и шум за окном.
                        # ⚠️ УВЕРЕННОСТЬ ВЫШЕ, ЧЕМ У ФИНАЛЬНОГО (0.5 против
                        # 0.25). Фон комнаты распознаётся как обрывки слов с
                        # низкой уверенностью — и помощник считал, что человек
                        # говорит, и молчал вечно. Поймано на стенде: при фоне
                        # 0.03 ответа не было вовсе.
                        if self.on_interim:
                            self.on_interim(transcript)
...
class GroqLLM:
    ...
    # ⚠️ 18.09 ИСПРАВЛЕН ДИАПАЗОН, КОТОРЫЙ СЪЕДАЛ ИЕРОГЛИФЫ.
    # Было: «\U000024C2-\U0001F251» одной строкой — «значки в рамках».
    # На деле этот промежуток тянется через ПОЛОВИНУ Юникода и забирает
    # заодно китайские иероглифы (4E00–9FFF), японские каны (3040–30FF) и
    # корейский (AC00–D7AF). Фильтр стоит только перед ОЗВУЧКОЙ, поэтому
    # на экране ответ был целым, а голосу доставалась одна латиница:
    # замер 18.09 — из китайского ответа съедалось 75%, из японского 78%,
    # 181 знак текста превращался в 5.7 секунды звука вместо тридцати.
    # Арабский, хинди, русский и английский лежат НИЖЕ 24C2 и не страдали —
    # поэтому беда и не всплывала, пока помощник не заговорил по-китайски.
    # Теперь вместо одного размашистого промежутка — три точных.
    EMOJI_PATTERN = re.compile(
        ...
        "\U000024C2-\U000024FF"  # Ⓜ и буквы в кружках
        "\U00002500-\U000025FF"  # ▪ ● ■ ► ◆ — маркеры списков и рамки.
                                 # ⚠️ ОБЯЗАТЕЛЬНО: их ел прежний широкий
                                 # диапазон, и без этой строки они посыпались
                                 # бы в голос — модель метит ими списки.
                                 # ⚠️ А вот 3000–303F (、。「」《》) НЕ ТРОГАЕМ:
                                 # это знаки препинания китайского и японского,
                                 # прежний диапазон ел и их — отсюда и рваная речь.
        "\U0001F000-\U0001F02F"  # маджонг
        "\U0001F0A0-\U0001F0FF"  # игральные карты
        "\U0001F100-\U0001F251"  # буквы и цифры в рамках
    ...
    def _build_messages(self, user_input: str, memory_ctx: str = "", web_ctx: str = ""):
        ...
        # 🌐 19.09 СЕТЬ ВЫКЛЮЧЕНА — ГОВОРИМ ОБ ЭТОМ ПОМОЩНИКУ.
        # ⚠️ ЗАЧЕМ: единственное место, где ему было сказано «погоду ты знать
        # не можешь», лежало ВНУТРИ описания инструмента. Инструмент мы больше
        # не даём — значит и запрет исчез, и на «какая погода» модель назовёт
        # правдоподобное число из головы. Выдуманный градус хуже паузы.
        # ⚠️ Строка вешается на тот же выключатель: вернёшь интернет — она
        # исчезнет сама, править ничего не надо.
        if not config.ИНТЕРНЕТ_ВКЛЮЧЁН:
            system += (
                "\n\n[СЕТИ У ТЕБЯ СЕЙЧАС НЕТ]: погоду, курсы валют, цены "
                "криптовалюты и новости мира ты посмотреть НЕ МОЖЕШЬ. "
                "Спросили о них — ответь честно и коротко: сейчас посмотреть "
                "не могу. ⛔ НИКАКИХ ЧИСЕЛ НЕ НАЗЫВАЙ — ни градусов, ни курса, "
                "ни цены, даже приблизительно и даже «обычно в это время года». "
                "Выдуманное число хуже честного «не знаю». Про свой сайт "
                "отвечай как всегда, из своих знаний: сеть ему не нужна."
            )
    ...
    async def generate_stream_turbo(self, user_input: str, memory_ctx: str = "",
        ...
        # 🌐 19.09 ИНСТРУМЕНТ ПОИСКА ДАЁМ, ТОЛЬКО ЕСЛИ ИНТЕРНЕТ ВКЛЮЧЁН.
        # Выключен — модель о нём не знает, звать нечего (см. настройки).
        if config.ИНТЕРНЕТ_ВКЛЮЧЁН:
            payload["tools"] = ИНСТРУМЕНТЫ
        ...
        _раздел = _какой_раздел_наверняка(user_input) if config.ИНТЕРНЕТ_ВКЛЮЧЁН else ""
        ...
        # 🌐 18.09 КОНЕЦ ПРЕДЛОЖЕНИЯ НА ЧУЖИХ ПИСЬМЕННОСТЯХ.
        # Китайскую и японскую точку «。！？» тут уже знали, а вот хинди
        # ставит в конце данду «।», араб — «؟» и «۔». Без них ответ на этих
        # языках не резался по предложениям вовсе и уходил в голос одним
        # длинным куском — отсюда рваная речь и паузы на длинном ответе.
        ...
        for attempt in range(groq_keys.total_keys + 2):   # +2 круга на вызов инструмента
            try:
                ...
                async with session.post(
                    ...
                    # 🌐 19.09 ВТОРОЙ ЗАМОК. Выше мы просто не даём модели
                    # инструмент — но если она всё же пришлёт вызов (своей
                    # волей, из памяти разговора, из чужого ответа), здесь
                    # код пошёл бы в сеть, зажёг значок «Ищу» и подарил ту
                    # самую паузу. Запираем и выход, не только вход.
                    if вызовы and not full_response.strip() and config.ИНТЕРНЕТ_ВКЛЮЧЁН:
                        try:
                            import web_router
                        except Exception:
                            web_router = None
                    ...
                    if not full_response.strip() and not chunk_buffer.strip():
                        # 🌐 19.09 ПОВТОР НУЖЕН И БЕЗ ИНСТРУМЕНТА. Проверка
                        # шла по наличию "tools" в запросе — а при выключенном
                        # интернете их там нет с самого начала, и вторая
                        # попытка стала мёртвой: модель промолчала — человек
                        # сразу получал отбивку «не расслышал». Молчит она и
                        # без инструментов: низкое усилие рассуждения съедает
                        # весь предел ответа. Повторяем в обоих случаях.
                        if payload.get("tools") or not config.ИНТЕРНЕТ_ВКЛЮЧЁН:
...
class EdgeTTSTurbo:
    # 🚦 27.08 ВОРОТА ДЛЯ MICROSOFT — не больше стольких кусков разом.
    # ⚠️ Замер 27.08: все разом — 0.6 с, но если пустить десяток, Edge
    # захлёбывается: часть не влезает в семь секунд, идут повторы, и один
    # кусок занимает до 22 секунд. Отсюда провалы в речи на длинном ответе.
    # У Яндекса такие ворота есть давно (шесть) — он держит без отказов.
    _ворота_edge = None
    """Edge TTS with streaming output for lower latency."""

    # 🚧 26.08 ОБЩИЕ НА ВЕСЬ ДВИЖОК, а не на сессию: если Яндекс лёг, это
    # касается всех разговоров разом, и каждому заново выяснять это незачем.

    _ya_отказов: int = 0        # сколько раз подряд не отдал звук
    _ya_пауза_до: float = 0.0   # до какого времени к нему не ходим

    @classmethod
    def _отметить_отказ(cls) -> None:
        """Посчитать отказ Яндекса и, если их много подряд, уйти в паузу."""
        cls._ya_отказов += 1
        if cls._ya_отказов >= config.YANDEX_ОТКАЗОВ_ДО_ПАУЗЫ:
            cls._ya_пауза_до = time.time() + config.YANDEX_ПАУЗА_СЕК
            logger.warning(
                f"🚧 Яндекс отказал {cls._ya_отказов} раза подряд — уходим "
    ...
    def __init__(self, lang: str = "ru"):
        ...
        _к = _код_языка(lang) or "ru"
        en = (_к != "ru")
        # родной голос для языков со своей письменностью (см. РОДНЫЕ_ГОЛОСА)
        _родной = РОДНЫЕ_ГОЛОСА.get(_к, "")
        self.voice = _родной or (config.TTS_VOICE_EN if en else config.TTS_VOICE)
    ...
    async def _yandex(self, text: str) -> bytes:
        ...
        # 🚧 Яндекс лёг совсем (кончился грант, отозван ключ)? Не тратим на
        # него по 4 секунды на каждой фразе — пережидаем и идём к Дмитрию.
        if EdgeTTSTurbo._ya_пауза_до > time.time():
            return b""
        ...
        # ⚡ 25.08 ПРОСИМ ВСЕ ПРЕДЛОЖЕНИЯ РАЗОМ, А НЕ ПО ОЧЕРЕДИ.
        # Было: попросил первое — подождал, попросил второе — подождал.
        # Пять предложений складывались в пять ожиданий подряд.
        # Замер на живом Яндексе (5 предложений, три круга):
        #   по очереди 4.15 · 1.83 · 2.21 с   →   разом 0.84 · 1.00 · 0.41 с
        # То есть в 2–5 раз быстрее, и чем длиннее ответ, тем больше выигрыш.
        # ⚠️ Порядок сохраняется: gather возвращает ответы в порядке задач,
        # поэтому склейка и паузы остаются ровно теми же. На слух — то же
        # самое, меняется только ожидание.
        # ⚠️ НЕ БОЛЬШЕ ШЕСТИ ЗАРАЗ. Обычный кусок — одно-три предложения, но
        # на длинном ответе их может набраться два десятка, и Яндекс вправе
        # притормозить за частоту. Шесть — с запасом быстрее, чем по очереди,
        # и без риска упереться в ограничение.
        ворота = asyncio.Semaphore(6)

        async def _один(к: str) -> bytes:
            async with ворота:
                return await self._ya_один(к)

        готовые = await asyncio.gather(
            *[_один(к) for к in куски], return_exceptions=True)

        собранное = []
        тишина = _КАДР_ТИШИНЫ * max(1, round(config.YANDEX_PAUSE_MS / _КАДР_МС))
        for i, звук in enumerate(готовые):
            # не отдали хоть одно предложение — уходим целиком, чтобы не
            # выдать человеку речь с дырой посередине
            if isinstance(звук, Exception) or not звук:
                return b""
            собранное.append(звук)
            if i < len(куски) - 1:
                собранное.append(тишина)
        # 🔚 27.08 ХВОСТ ТИШИНЫ В КОНЦЕ — КАК У ДМИТРИЯ.
        # ⚠️ Замер на одном тексте показал: начало и паузы внутри у голосов
        # СОВПАДАЮТ (0.25 ←→ 0.27 и 1.09/1.20 ←→ 1.27/1.20), а конец — нет.
        #     Дмитрий: 0.93 с выдоха в конце
        #     Ермил:   0.29 с — обрывается вчетверо резче
        # Из-за этого ответ звучал как оборванный: человек не успевал понять,
        # что помощник договорил, и казалось, будто он «пропал».
        # Дописываем недостающие 0.64 с — теми же кадрами, что и паузы между
        # предложениями. Числа не выдуманы: они взяты у Дмитрия, на котором
        # всё было выверено месяцами.
        # ⚠️ Хвост платится ОДИН раз, в самом конце ответа, а не на каждом
        # куске — иначе растянулась бы вся речь.
        хвост = _КАДР_ТИШИНЫ * max(0, round(config.YANDEX_ХВОСТ_МС / _КАДР_МС))
        return b"".join(собранное) + хвост
    ...
    async def _ya_один(self, text: str) -> bytes:
        ...
        # 🔁 26.08 ДВЕ ПОПЫТКИ. Против сбоя связи, а не против падения службы:
        # если Яндекс лёг, повтор упрётся в то же самое, а моргнувшую сеть
        # вылечит. Ошибку в ключе или в запросе (4xx) НЕ повторяем — она
        # никуда не денется, только съест время до Дмитрия.
        звук = b""
        for попытка in range(1, config.YANDEX_TRIES + 1):
            try:
                ...
                if 400 <= e.code < 500:
                    logger.warning(f"🗣️ Яндекс отказал ({e.code}) — повтор не поможет")
                    # ⚠️ Именно такой отказ и означает «лёг совсем»: кончился
                    # грант, отозван ключ. Считаем его, иначе защита никогда
                    # не включится — а нужна она ровно в этом случае.
                    EdgeTTSTurbo._отметить_отказ()
                    return b""
                logger.warning(f"🗣️ Яндекс: ошибка {e.code}, попытка {попытка}")
            except Exception as e:
                logger.warning(f"🗣️ Яндекс молчит ({type(e).__name__}), "
                               f"попытка {попытка} из {config.YANDEX_TRIES}")
        if not звук:
            EdgeTTSTurbo._отметить_отказ()
            return b""
        EdgeTTSTurbo._ya_отказов = 0   # отдал звук — счётчик обнуляем
    ...
    async def synthesize(self, text: str) -> bytes:
        ...
        # 🔀 27.08 РУССКАЯ СТОРОНА — ТОЛЬКО ЯНДЕКС.
        # Его решение: подстраховку Дмитрием ВЫКЛЮЧИТЬ. Яндекс не ответил —
        # тишина, без метаний. Так проще держать одну дорожку и сразу видно,
        # если служба легла.
        # ⚠️ ВЕРНУТЬ ПОДСТРАХОВКУ = поставить ПОДСТРАХОВКА_ДМИТРИЕМ = True
        # в настройках. Весь код Дмитрия цел и работает, он только не зовётся.
        сырой = text
    ...
    async def synthesize_streaming(self, text: str, send_callback) -> None:
        ...
        """
        # 🔀 27.08 РУССКАЯ СТОРОНА — ТОЛЬКО ЯНДЕКС (см. пояснение в synthesize).
        сырой = text
        if self.ya_on:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 🚦 ворота: не больше нескольких кусков разом, иначе Edge
                # захлёбывается и срабатывают повторы по семь секунд.
                if EdgeTTSTurbo._ворота_edge is None:
                    EdgeTTSTurbo._ворота_edge = asyncio.Semaphore(config.EDGE_РАЗОМ)
                # 🛡️ сторож: попытка не длится дольше config.TTS_TIMEOUT
                async with EdgeTTSTurbo._ворота_edge:
                    звук = await asyncio.wait_for(
                        self._collect(communicate), timeout=config.TTS_TIMEOUT
                    )
...
# 🌐 18.09 приветствия на прочих девяти языках: берутся из хранилища при
# первом госте на этом языке и дальше живут в памяти до перезапуска
...
# 🔊 18.09 ГОТОВАЯ ЗАПИСЬ ПРИВЕТСТВИЯ ИЗ ХРАНИЛИЩА.
# Вынесена на уровень модуля: её зовёт и подготовка при старте,
# и обработчик /api/greeting. Одиннадцать приветствий лежат там
# готовыми файлами — озвучивать их заново больше не нужно.
...
async def warm_greetings():
    ...
    try:
        ...
        if not CACHED_GREETING_AUDIO:
            ...
            откуда_ru = "синтез"
        # 🌐 18.09 АНГЛИЙСКОЕ — СНОВА ИЗ ХРАНИЛИЩА. Туда положена новая
        # запись, со строкой о многих языках, и она проверена. Синтез
        # остаётся лишь запасным путём, на случай если файл не скачался.
        CACHED_GREETING_AUDIO_EN = await _из_хранилища("en")
        откуда_en = "хранилище"
        if not CACHED_GREETING_AUDIO_EN:
            logger.warning("VOICE: английское приветствие из хранилища не взялось — озвучиваю сам")
            CACHED_GREETING_AUDIO_EN = await EdgeTTSTurbo("en").synthesize(config.GREETING_TEXT_EN)
            откуда_en = "синтез (запасной путь)"
...
async def get_greeting(request: Request, lang: str = "ru"):
    ...
    голос стал основным везде, и приветствие должно ему соответствовать.
    ...
    if _к == "ru":
        ...
        # 🌐 18.09 ОСТАЛЬНЫЕ ДЕВЯТЬ ЯЗЫКОВ — ГОТОВЫМИ ЗАПИСЯМИ ИЗ ХРАНИЛИЩА — как английская и русская.
        # Берём один раз на язык и держим в памяти, чтобы не ходить туда на
        # каждого гостя. Файла нет — озвучиваем сами, запасным путём.
        audio = ПРИВЕТ_КЕШ.get(_к, b"")
        if not audio:
            audio = await _из_хранилища(_к)
            if audio:
                ПРИВЕТ_КЕШ[_к] = audio
                logger.info(f"VOICE: приветствие {_к} взято из хранилища ({len(audio)} байт)")
        if not audio:
            try:
                logger.warning(f"VOICE: приветствия {_к} нет в хранилище — озвучиваю сам")
...
async def voice_health():
    ...
    return {
        ...
        "сборка": "2026-09-19 Джон + английский старт",
        "llm": current_model(),
        "stt": "Deepgram Nova-3",
...
class VoiceSessionTurbo:
    ...
    def __init__(self, websocket: WebSocket, session_id: str, client_ip: str = ""):
        ...
        # 🎧 запись текущей реплики — её слушает Smart Turn.
        # ⚠️ Держим только последние 8 секунд: больше модель не берёт, а
        # память на длинном разговоре росла бы без нужды.
        self._звук_реплики = bytearray()
        self._ответ_задача = None      # ⏱️ живая задача ответа (для сторожа)
    ...
    async def handle_audio(self, audio_data: bytes):
        ...
        try:
            ...
            pass
        # 🎧 копим звук реплики — его слушает Smart Turn.
        # ⚠️ Держим только последние 8 секунд: больше модель не берёт
        # (сама обрезает), а память на длинном разговоре росла бы зря.
        if SMART_TURN:
            self._звук_реплики.extend(audio_data)
            _предел = 16000 * 2 * 9          # 9 секунд, 16 кГц, 2 байта
            if len(self._звук_реплики) > _предел:
                del self._звук_реплики[:-_предел]
    ...
    # 🩹 28.08 СКОЛЬКО ЖИВЁТ СТОРОЖ, секунды.
    # ⚠️ Было десять витков по секунде, и они кончались на длинном ответе —
    # реплика оставалась висеть. Сорок секунд с запасом переживают самый
    # долгий ответ. Срок сдвигается, пока человек говорит.
    СТОРОЖ_ЖИВЁТ = 40.0
    ...
    def _schedule_autoflush(self):
        ...
        async def _later():
            try:
                # 🩹 27.08 СТОРОЖ СЛЕДИТ, А НЕ ПРОВЕРЯЕТ ОДИН РАЗ.
                # ⚠️ Зачем, по его журналу 26.08: реплика «Будем читать?»
                # пришла в тот самый миг, когда прошлый ответ дорабатывал, и
                # пролежала в буфере 39 СЕКУНД — помощник молчал, пока человек
                # не переспросил «ты завис?». Разовая проверка сорвалась, а
                # второго шанса не было: браузер сигнал «договорил» дважды не
                # шлёт, а распознавание нового текста не пришлёт, пока человек
                # молчит.
                # Повторяющийся сторож не зависит от того, ПОЧЕМУ сорвалась
                # первая попытка. Цена: одна проверка в секунду, десять раз.
                # 🩹 28.08 СТОРОЖ ЖИВЁТ ПО ЧАСАМ, А НЕ ПО ВИТКАМ.
                # ⚠️ Его журнал 28.08: реплика «Зашел пообщаться с тобой»
                # пришла в 08:10:44, ответ на прежнюю шёл до 08:10:54 — и
                # сторож выдохся за миг до того, как освободилось место.
                # Реплика осталась необработанной, в окне повисла серая
                # строка, помощник промолчал.
                # Причина: витков было десять, и каждый тратился ДАЖЕ когда
                # сторож просто ждал молчания человека. На длинном ответе
                # они кончались впустую.
                # Теперь сторож держится ПО ВРЕМЕНИ — сорок секунд, — и
                # ожидание молчания витков не съедает.
                await asyncio.sleep(self.СРОК_ОЖИДАНИЯ)
                _до_когда = time.time() + self.СТОРОЖ_ЖИВЁТ
                while time.time() < _до_когда:
                    if not self.is_active:
                        return
                    if not self.transcript_buffer.strip():
                        return           # реплику уже забрали — всё хорошо
                    # Смотрим на ЖИВУЮ задачу, а не только на флаг: флаг
                    # снимается лишь у актуального ответа и может залипнуть.
                    занят = (self.is_processing
                             and getattr(self, "_ответ_задача", None) is not None
                             and not self._ответ_задача.done())
                    # 🤫 27.08 СТОРОЖ ТОЖЕ ЖДЁТ МОЛЧАНИЯ.
                    # ⚠️ Иначе он подхватывал реплику через секунду, минуя
                    # придержку, — и помощник отвечал, хотя человек ещё
                    # говорил. Сторож для другого: он спасает реплику,
                    # ЗАЛЕЖАВШУЮСЯ в буфере, а не торопит живую речь.
                    _тихо = time.time() - getattr(self, "_миг_звука", 0.0)
                    if _тихо < self.ЖДАТЬ_КОНЦА_МЫСЛИ:
                        # человек ещё говорит — ждём, но срок сторожа сдвигаем:
                        # он должен пережить всю речь и ответ на неё
                        _до_когда = time.time() + self.СТОРОЖ_ЖИВЁТ
                        await asyncio.sleep(1.0)
                        continue
                    if not занят:
                        if self.is_processing:
                            logger.warning(f"[{self.session_id}] 🩹 флаг завис — снимаю")
                            self.is_processing = False
                            self._processing_since = 0
                        logger.info(f"[{self.session_id}] 🛟 Браузер молчит — обрабатываю сам")
                        note(self.session_id, "🛟 подхватил залежавшуюся реплику",
                             self.transcript_buffer[:70])
                        self._ответ_задача = asyncio.create_task(self.on_speech_end())
                        return
                    await asyncio.sleep(1.0)
            except asyncio.CancelledError:
                pass
                ...
                note(self.session_id, "⚠️ страховка сорвалась", str(e)[:70])
    ...
    # 🤫 27.08 СКОЛЬКО МОЛЧАНИЯ ЖДАТЬ, ПРЕЖДЕ ЧЕМ ОТВЕЧАТЬ.
    # ⚠️ Его число и его схема: пока говоришь с паузами — помощник копит
    # и склеивает, не открывая рта. Замолчал на этот срок — отвечает на
    # всё разом, одной мыслью.
    # Цена: короткий вопрос тоже ждёт этот срок. Меньше — длинная речь
    # начнёт рваться на куски, как было.
    ЖДАТЬ_КОНЦА_МЫСЛИ = 4.0

    # С какой длины фраза считается короткой (ждёт быстро) и с какой —
    # длинной мыслью (ждёт полный срок). Между ними ожидание растёт плавно.
    # Сколько лишнего подождать, если слова встали, а в микрофоне речь.
    # ⚠️ Мало намеренно: микрофон здесь не судья, а подсказка. Больше —
    # и шум за окном начнёт держать помощника.
    ДОБАВКА_ПО_ЗВУКУ = 1.2

    КОРОТКАЯ_ФРАЗА_ЗНАКОВ = 20
    ДЛИННАЯ_МЫСЛЬ_ЗНАКОВ = 100
    ...
    def _on_transcript(self, text: str):
        ...
        # 🔑 27.08 ОТМЕЧАЕМ КАЖДОЕ УСЛЫШАННОЕ СЛОВО — от него считается окно.
        self._последнее_слово = time.time()
    ...
    async def перебили(self, откуда: str = "браузер"):
        """🛑 Умолкнуть: человек заговорил поверх ответа.

        ⚠️ Порог перебивания стоит В БРАУЗЕРЕ, и не по громкости, а по
        ВРЕМЕНИ: чтобы оборвать помощника, надо говорить непрерывно 0.75 с
        на компьютере и 1.3 с на телефоне. Так сделано потому, что микрофон
        слышит динамик — без этого помощник перебивал бы сам себя.
        Кашель длится треть секунды и в порог не влезает.
        """
        self.barge_in_requested = True
        note(self.session_id, "ПЕРЕБИЛИ", откуда)
        self.is_speaking = False
        self.is_processing = False
        self.transcript_buffer = ""
        logger.info(f"[{self.session_id}] 🛑 Interrupted ({откуда})")

    async def _слышит_ли_конец(self):
        """🎧 Спросить Smart Turn: договорил человек или взял паузу?

        Возвращает True (договорил), False (не договорил) или None, если
        спросить не вышло — тогда работаем по числам, как раньше.

        ⚠️ Модель слушает ЗВУК, а не текст: она ловит интонацию — падает
        голос в конце или висит. Потому и видит то, чего не видно в
        расшифровке.
        """
        if not SMART_TURN or len(self._звук_реплики) < 16000 * 2:
            return None                     # меньше секунды звука — не спрашиваем
        try:
            звук = _np.frombuffer(bytes(self._звук_реплики),
                                  dtype=_np.int16).astype(_np.float32) / 32768.0
            р = await asyncio.get_event_loop().run_in_executor(
                None, _слышит_конец, звук)
            договорил = bool(р.get("prediction"))
            уверен = float(р.get("probability", 0))
            logger.debug(f"[{self.session_id}] 🎧 модель: "
                         f"{'договорил' if договорил else 'ждёт'} ({уверен:.2f})")
            return договорил
        except Exception as e:
            logger.warning(f"[{self.session_id}] 🎧 модель не ответила: {e}")
            return None

    def _на_поток(self, text: str):
        """🌊 27.08 ПОТОКОВЫЙ ТЕКСТ — слова, пока человек ещё говорит.

        ⚠️ ГЛАВНОЕ, ЧТО ОН ДАЁТ: точный признак «человек говорит».
        Раньше это определялось громкостью микрофона — а её поднимают кашель,
        стук, голос из телевизора, и помощник молчал впустую. Слова же шум не
        рождает: они приходят, только когда есть распознанная речь.

        ⚠️ В БУФЕР НЕ КЛАДЁМ. Поток уточняется на ходу («прив» → «привет» →
        «привет дружище»), и в реплику попал бы мусор. Он только показывает
        человеку его слова и отмечает время последнего.
        """
        if not text or not self.is_active:
            return
        self._миг_потока = time.time()
        self._поток_текст = text
        # 🌊 27.08 ШЛЁМ ВСЮ РЕПЛИКУ ЦЕЛИКОМ, А НЕ ГОЛЫЙ ОТРЕЗОК.
        # ⚠️ Распознавание шлёт не одну растущую строку, а отрезки: каждые
        # несколько секунд закрывает прежний и начинает новый с нуля. Замер
        # 27.08: «…Как поживаешь? Что» → «Что нового у тебя?» → «Хотелось».
        # Если слать голый отрезок, в окне текст пропадает и появляется
        # заново — это и было дёрганье.
        # Здесь у нас есть ПОЛНЫЙ буфер уже закрытых отрезков: складываем
        # его с текущим и шлём непрерывную строку. Браузеру остаётся только
        # показать её — ничего склеивать не нужно.
        целиком = self._приклеить(self.transcript_buffer, text)
        # 🔢 28.08 ЧИНИМ ЦИФРЫ И В ОКНЕ, А НЕ ТОЛЬКО В РАБОТЕ.
        # ⚠️ Его замечание: в окне писалось «на 1 волне», хотя в ответ уходило
        # уже «на одной волне». Починка стояла дальше по пути, а поток идёт
        # раньше — и человек видел неправленый текст.
        целиком = _починить_распознанное(целиком)
        asyncio.create_task(self._send_json({
    ...
    async def on_speech_end(self, from_browser: bool = False):
        ...
        # ⏳ 27.08 ЖДЁМ, ПОКА РАСПОЗНАВАНИЕ ДОМОЛЧИТ.
        # ⚠️ Его беда, пойманная на долгом разговоре: длинная реплика уходила
        # в работу на 192 знаках, а хвост «Так что ответь мне» приходил
        # позже — и становился ОТДЕЛЬНОЙ репликой, прилипая к следующему
        # вопросу. Отсюда «Так что ответь мне. Привет.» вместо «Привет».
        # Причина: модель говорит «договорил», мы берём буфер, а Deepgram
        # ещё досылает окончательные куски.
        # Ждём короткую тишину в ПОТОКЕ ТЕКСТА — не в микрофоне. Цена —
        # доля секунды, зато реплика уходит целой.
        _ждали = 0.0
        while _ждали < 1.0:
            _с_последнего = time.time() - getattr(self, "_последнее_слово", 0)
            if _с_последнего >= 0.5:
                break
            await asyncio.sleep(0.05)
            _ждали += 0.05
        ...
        if not transcript:
            ...
            # 🩹 27.08 НЕ УХОДИМ МОЛЧА — ЖДЁМ ТЕКСТ.
            # ⚠️ Его беда, слово в слово: «если как-то криво скажешь, он
            # замолкал, и потом только снова надо спрашивать».
            # Вот отчего: браузер объявляет «договорил», а распознавание ещё
            # не отдало текст — он приходит через полсекунды и ложится в
            # пустой буфер. А забрать его некому: браузер второй раз сигнал
            # не пошлёт, и реплика лежит мёртвым грузом.
            # Теперь заводим сторож: придёт текст в ближайшие секунды —
            # подхватим сами и ответим.
            self._schedule_autoflush()
            return
        ...
        # 🔑 27.08 ОКНО СКЛЕЙКИ — ОТ ПОСЛЕДНЕГО СЛОВА ЧЕЛОВЕКА.
        # ⚠️ Его находка: раньше окно отсчитывалось от мига, когда взяли
        # реплику в работу, — и длинная речь резалась на седьмой секунде,
        # сколько ни говори. Теперь говорить можно хоть три минуты.
        _отсчёт = max(getattr(self, "_взяли_в_работу", 0),
                      getattr(self, "_последнее_слово", 0))
        _прошло_с_реплики = time.time() - _отсчёт
        ...
        try:
            ...
            # 🩹 27.08 ПОСЛЕДНИЙ ВЗГЛЯД В БУФЕР ПЕРЕД ОТВЕТОМ.
            # ⚠️ Его беда 27.08: «Понял, дружище, а что в Вашингтоне творится?»
            # ушло в работу, а «Какая погода? Какие новости?» пришло РОВНО в
            # тот же миг — на доли секунды позже. Оно осталось в буфере и
            # приклеилось к следующему вопросу, а помощник ответил, не
            # услышав половины.
            # Ожидание тишины перед взятием не спасло: текст пришёл впритык.
            # Здесь смотрим ещё раз, у самой двери: появилось что-то — клеим
            # к своей реплике и отвечаем на целое.
            if self.transcript_buffer.strip():
                _добавка = self.transcript_buffer.strip()
                self.transcript_buffer = ""
                transcript = self._приклеить(transcript, _добавка)
                note(self.session_id, "догнало у двери", _добавка[:60])
                logger.info(f"[{self.session_id}] 🩹 текст догнал у двери: {_добавка[:50]}")
            ...
            if _мин_придержки > 0:
                ...
                # 🤫 27.08 ПОТОЛОК СЧИТАЕМ ОТ ПОСЛЕДНЕГО СЛОВА ЧЕЛОВЕКА.
                # ⚠️ Иначе он истекал посреди длинной речи: человек говорит
                # двадцать секунд, а потолок отмерен от начала придержки —
                # и помощник влезал с ответом на полуслове.
                # Теперь потолок сдвигается с каждым словом, а держит его
                # только настоящее молчание.
                while True:
                    _посл = max(_старт_придержки, getattr(self, "_последнее_слово", 0))
                    if time.time() >= _посл + self.ПРИДЕРЖКА_ПОТОЛОК:
                        break
                    ...
                    # 🤫 27.08 КОПИМ, ПОКА ЧЕЛОВЕК ГОВОРИТ (его схема, дословно):
                    # «склеивал и склеивал, пока я говорю; и только когда я
                    # замолчал на четыре секунды — понимал, что разговор
                    # закончен».
                    # Значит ждём именно МОЛЧАНИЯ на срок, а не времени с
                    # начала придержки. Пока человек говорит с паузами — копим
                    # и склеиваем, не отвечая. Замолчал — отвечаем на всё разом.

                    # 🤫 27.08 ЖДЁМ НЕ ПУСТОГО БУФЕРА, А ТОГО, ЧТО ЧЕЛОВЕК
                    # ЗАМОЛЧАЛ.
                    # ⚠️ Раньше стояло «буфер пуст» — и выходил замкнутый
                    # круг: буфер не пустеет, пока реплику не заберут, а
                    # забрать нельзя, пока буфер не пуст. Разрывался только
                    # потолком, и человек ждал ответа все десять секунд.
                    # Теперь смотрим, давно ли пришло последнее слово: пока
                    # человек говорит — копим и склеиваем; замолчал на срок —
                    # отвечаем на всё разом. Ровно его схема.
                    # 🌊 27.08 СУДИМ ПО ПОТОКУ СЛОВ, А НЕ ПО ГРОМКОСТИ.
                    # ⚠️ Роли разведены, чтобы не спорили:
                    #   ТЕКСТ РЕШАЕТ — слова идут, значит человек говорит;
                    #     встали, значит замолчал, и пошёл отсчёт.
                    #   МИКРОФОН ТОЛЬКО ПРИДЕРЖИВАЕТ — он не может сказать
                    #     «человек говорит», он говорит лишь «подожди ещё
                    #     чуть». И это «чуть» ограничено ДОБАВКА_ПО_ЗВУКУ,
                    #     иначе шум за окном держал бы помощника вечно.
                    # Зачем микрофон вообще нужен: если распознавание
                    # задумалось или связь моргнула, поток слов встанет, а
                    # человек будет говорить. Эта добавка даст словам догнать.
                    _посл_слово = max(getattr(self, "_последнее_слово", 0),
                                      getattr(self, "_миг_потока", 0))
                    _молчит = time.time() - _посл_слово
                    # 🎚️ 27.08 ЖДЁМ ПО ДЛИНЕ СКАЗАННОГО — ШКАЛОЙ, НЕ СТУПЕНЬКОЙ.
                    # ⚠️ Его задача: «на коротких — привет, как дела — отвечать
                    # мгновенно; а если говорю долго, ждать три-четыре секунды».
                    # Человек, сказавший пару слов, ждёт быстрого ответа. Кто
                    # начал излагать — тот продолжит, и ему нужна пауза подумать.
                    # Растёт плавно: 15 знаков → 0.5 с, 100 знаков → 4 с.
                    # ⚠️ Если уже была склейка — мысль точно длинная, ждём
                    # полный срок независимо от длины куска.
                    # 🎧 27.08 СПРАШИВАЕМ МОДЕЛЬ, А НЕ ГАДАЕМ ПО ДЛИНЕ.
                    # Она слушает интонацию последних секунд речи и говорит,
                    # договорил человек или взял паузу посреди мысли.
                    # ⚠️ Спрашиваем ТОЛЬКО когда человек уже замолчал: модель
                    # плохо читает очень короткие обрывки, ей нужен разбег.
                    # ⚠️ Не чаще раза в 0.4 с: сорок миллисекунд на решение,
                    # но в цикле каждые 0.05 с это была бы лишняя нагрузка.
                    if (SMART_TURN and _молчит >= 0.3
                            and time.time() - getattr(self, "_спросили_модель", 0) > 0.4):
                        self._спросили_модель = time.time()
                        _реш = await self._слышит_ли_конец()
                        if _реш is not None:
                            self._модель_сказала = _реш

                    _знаков = len(self._последняя_реплика or transcript)
                    _мнение = getattr(self, "_модель_сказала", None)
                    if _мнение is False:
                        # модель слышит: человек НЕ договорил — ждём полный срок
                        _ждём_молчания = self.ЖДАТЬ_КОНЦА_МЫСЛИ
                    elif _мнение is True and not getattr(self, "_склеек_подряд", 0):
                        # модель слышит: договорил — отвечаем быстро.
                        # ⚠️ 27.08 НО ТОЛЬКО ЕСЛИ СКЛЕЙКИ ЕЩЁ НЕ БЫЛО.
                        # Модель ошибается тем чаще, чем длиннее речь: замер
                        # на живой английской (19 пауз) — сказала «договорил»
                        # 7 раз посреди мысли; на русской (14 пауз) — 2 раза.
                        # Если склейка уже случилась, человек ТОЧНО говорит
                        # длинную мысль с паузами — и обрывать его на полслове
                        # по одному лишь мнению модели нельзя. Ждём полный срок.
                        _ждём_молчания = self.ПРИДЕРЖКА_ЗАКОНЧЕННОЙ
                    elif getattr(self, "_склеек_подряд", 0):
                        _ждём_молчания = self.ЖДАТЬ_КОНЦА_МЫСЛИ
                    else:
                        _доля = min(1.0, max(0.0, (_знаков - self.КОРОТКАЯ_ФРАЗА_ЗНАКОВ)
                                             / float(self.ДЛИННАЯ_МЫСЛЬ_ЗНАКОВ
                                                     - self.КОРОТКАЯ_ФРАЗА_ЗНАКОВ)))
                        _ждём_молчания = (self.ПРИДЕРЖКА_ЗАКОНЧЕННОЙ
                                          + _доля * (self.ЖДАТЬ_КОНЦА_МЫСЛИ
                                                     - self.ПРИДЕРЖКА_ЗАКОНЧЕННОЙ))
                    # микрофон добавляет, но не блокирует: если слова встали,
                    # а в микрофоне явная речь — ждём ещё, но не дольше добавки
                    if _тихо_уже < _нужно_тиши:
                        _молчит = min(_молчит,
                                      _ждём_молчания + self.ДОБАВКА_ПО_ЗВУКУ - 0.01)
                    if (time.time() >= _держим_мин
                            and _молчит >= _ждём_молчания):
            ...
            # ⏩ 27.08 КОНВЕЙЕР ОЗВУЧКИ: очередь мест и запущенные задачи.
            # Готовим следующий кусок, пока играет нынешний, а отправляем
            # строго по порядку — иначе речь перепуталась бы.
            _очередь_озвучки = []
            _задачи_озвучки = []
            ...
            async for text_chunk in self.llm.generate_stream_turbo(
                ...
                # 🔊 ПОТОМ озвучка.
                # ⏩ 27.08 ОЗВУЧКА ИДЁТ ВПЕРЁД, НЕ ДОЖИДАЯСЬ ОТПРАВКИ.
                # ⚠️ Его беда на длинном ответе: помощник говорил, замолкал
                # на несколько секунд, снова говорил. По дневнику 27.08 такой
                # ответ шёл 99 секунд с провалами.
                # Причина была здесь: мы ЖДАЛИ озвучку каждого куска и лишь
                # потом брали следующий. Озвучка занимает секунду-полторы, и
                # на длинном ответе это набегает в тишину.
                # Теперь готовим следующий кусок, пока играет нынешний.
                # ⚠️ ОТПРАВКА СТРОГО ПО ПОРЯДКУ: куски ждут своей очереди,
                # иначе речь перепуталась бы.
                мой_черёд = _очередь_озвучки[-1] if _очередь_озвучки else None
                моё_место = asyncio.Event()
                _очередь_озвучки.append(моё_место)

                async def _озвучить(текст=text_chunk_tts, жду=мой_черёд, моё=моё_место):
                    собрано = []
                    async def копить(звук):
                        собрано.append(звук)
                    try:
                        await self.tts.synthesize_streaming(текст, копить)
                    except Exception as e:
                        logger.warning(f"[{self.session_id}] озвучка куска не вышла: {e}")
                    # ждём, пока отговорит предыдущий кусок
                    if жду is not None:
                        try:
                            await asyncio.wait_for(жду.wait(), timeout=30)
                        except asyncio.TimeoutError:
                            pass
                    for звук in собрано:
                        await send_audio(звук)
                    моё.set()

                _задачи_озвучки.append(asyncio.create_task(_озвучить()))
            
            # ⏩ дожидаемся, пока все куски отговорят
            if _задачи_озвучки:
                try:
                    await asyncio.wait_for(
                        asyncio.gather(*_задачи_озвучки, return_exceptions=True),
                        timeout=90)
                except asyncio.TimeoutError:
                    logger.warning(f"[{self.session_id}] озвучка не уложилась в срок")
            ...
            if _я_актуален():
                ...
                self._склеек_подряд = 0   # мысль закончена — счёт заново
                # 🎧 забываем запись и мнение модели: началась новая реплика
                self._звук_реплики = bytearray()
                self._модель_сказала = None
                self.is_processing = False
                self.is_speaking = False
                self._processing_since = 0
...
async def websocket_voice(websocket: WebSocket):
    ...
    if _стр == "ru":
        ...
        # 🇬🇧 19.09 АНГЛИЙСКАЯ СТРАНИЦА НАЧИНАЕТ ПО-АНГЛИЙСКИ (его решение).
        # Русский язык браузера тут не берём: с ним сессия стартовала бы
        # русской, а русский старт правило «язык по тексту» не трогает —
        # и гость с русским браузером получал русский ответ даже на
        # английский вопрос (поймано 19.09). Заговорит по-русски — правило
        # по тексту переключит на русский и Ермила само, как для всех.
        if session.lang == "ru":
            session.lang = "en"
                    ...
                    if "bytes" in message and message["bytes"]:
                        ...
                        try:
                            ...
                            if cmd == "ping":
                                ...
                                # ⏱️ помним задачу: по ней сторож узнаёт, живы ли мы
                                session._ответ_задача = asyncio.create_task(
                                    session.on_speech_end(from_browser=True))
                                ...
                                await session.перебили("браузер")
