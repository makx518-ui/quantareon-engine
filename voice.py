...
ИНСТРУМЕНТЫ = [{
    ...
    "function": {
        ...
        "description": (
            ...
            "🌡 ПОГОДА — ВСЕГДА ЧЕРЕЗ ЭТОТ ИНСТРУМЕНТ, БЕЗ ИСКЛЮЧЕНИЙ. Ты не "
            "знаешь погоду ни в одном городе мира, даже в том, где сам "
            "находишься, даже если только что называл её в этом разговоре — "
            "она меняется каждый час. Спросили про погоду — вызывай. Назвать "
            "числа, не вызвав инструмент, — грубая ошибка: человек поверит "
            "выдуманному.\n\n"
            ...
            "properties": {
                ...
                "город": {
                    ...
                    "description": (
                        ...
                        "🌍 ГОРОДОВ МОЖЕТ БЫТЬ НЕСКОЛЬКО — перечисли их через "
                        "запятую в ОДНОМ вызове, второй раз звать не нужно. "
                        "«погода в Лондоне и Париже» → «Лондон, Париж». "
                        "Ответ придёт по каждому городу отдельно."
                    ),
                },
            },
...
class GroqLLM:
    ...
    async def generate_stream_turbo(self, user_input: str, memory_ctx: str = "",
        ...
        if _нужен_интернет_наверняка(user_input) and not web_ctx:
            payload["tool_choice"] = {
                "type": "function",
                "function": {"name": "поиск_в_интернете"},
            }
            logger.info(f"[{self.session_id}] 🌐 интернет обязателен для этой реплики")
...
async def voice_health():
    ...
    return {
        ...
        "сборка": "2026-08-16 интернет-обязателен",
        "llm": current_model(),
        "stt": "Deepgram Nova-3",
