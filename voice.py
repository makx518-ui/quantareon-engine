...
class VoiceSessionTurbo:
    ...
    async def on_speech_end(self, from_browser: bool = False):
        ...
        # 🧩 ОКНО ГОНКИ. Между «оборвали прошлый ответ» и «запустили новый»
        # флаг is_processing на доли секунды снят. Продолжение, попавшее ровно
        # туда, склейки не находило — поймано в песочнице 12.08 на пробе
        # «три куска подряд»: третий кусок ушёл отдельной репликой.
        # Поэтому считаем себя занятыми ещё ХВОСТ_ЗАНЯТОСТИ секунд после конца.
        _только_что = (time.time() - getattr(self, "_ответ_кончился", 0)) < self.ХВОСТ_ЗАНЯТОСТИ

        _в_щели = getattr(self, "_передаём_ход", False)

        if self.is_processing or _в_щели or _только_что:
            ...
            # было 13, стало 0.
            прошло = time.time() - getattr(self, "_взяли_в_работу", 0)
            if прошло <= self.ОКНО_СКЛЕЙКИ and getattr(self, "_последняя_реплика", ""):
                ...
                note(self.session_id, "СКЛЕЙКА",
                     f"через {прошло:.1f}с · «{transcript[-60:]}»")
                logger.info(f"[{self.session_id}] 🧩 Склеил с прошлой репликой ({прошло:.1f}с)")
            else:
