...
class Config:
    ...
    TTS_RATE: str = "+0%"   # 08.08 по его слову: «сделай ноль» — тараторил
    TTS_PITCH: str = "-9Hz"
    TTS_VOLUME: str = "+9%"
...
class VoiceSessionTurbo:
    ...
    async def on_speech_end(self, from_browser: bool = False):
        ...
        # 🧩 РЕШАЕТ ВРЕМЯ ОТ ВЗЯТИЯ РЕПЛИКИ, А НЕ «ЗАНЯТ ЛИ СЕЙЧАС».
        # ⚠️ ЖИВОЙ ПРОВАЛ 12.08, СТОИВШИЙ ДЕПЛОЯ. Сначала здесь стояло
        # `if self.is_processing or ...` — и на КОРОТКОЙ фразе склейка не
        # срабатывала: ответ успевал отзвучать раньше, чем приходило
        # продолжение, сервер уже был свободен, условие не выполнялось.
        # Он поймал это первым же разговором: «Я рад, что у тебя всё хорошо.
        # Я пока» ушло одной репликой, «тестирую голосового помощника» —
        # другой.
        # ⚠️ Мой стенд этого поймать НЕ МОГ: там ответ всегда длился 3 секунды
        # и не успевал закончиться. Модель и код разошлись именно здесь.
        # Теперь смотрим только на время: прошло ли меньше ОКНО_СКЛЕЙКИ с
        # момента, когда взяли прошлую реплику в работу. Занят сервер или уже
        # свободен — неважно, важно, что человек ДОГОВАРИВАЕТ ту же мысль.
        ...
        try:
            ...
            сказано = []
            оборвали = False
            вопрос_на_который_отвечали = transcript   # 🧩 запомнили до всякой склейки
            перебили_голосом = False
            ...
            async for text_chunk in self.llm.generate_stream_turbo(
                ...
                if not self.is_active or self.barge_in_requested:
                    # 🎤 перебил голосом или пришла новая реплика? от этого
                    # зависит, какую метку класть в память
                    перебили_голосом = not getattr(self, "_передаём_ход", False)
                    self.barge_in_requested = False
                    оборвали = True
                    break
            ...
            #
            # 🧩 НО НЕ ПРИ СКЛЕЙКЕ. Если человек просто договаривал свою мысль,
            # наш оборванный ответ был ответом на ОГРЫЗОК — он неверен по сути
            # и в память ему не место. Через мгновение мы ответим на целую
            # фразу, и вот это и есть настоящий ответ.
            # ⚠️ Без этой проверки история забивается дублями: ответ на огрызок
            # + ответ на целое, и так на каждой паузе. Через несколько склеек
            # помощник начинает ссылаться на то, чего не говорил.
            # 🧩 Этот ответ оборвали РАДИ СКЛЕЙКИ? Сравниваем по номеру реплики,
            # а не по флагу. ⚠️ Флаг залипал: если ответ успевал завершиться
            # нормально, до этого места дело не доходило, флаг оставался
            # поднятым — и следующее НАСТОЯЩЕЕ перебивание не сохранялось в
            # память. Поймано четырьмя голосами 12.08.
            ради_склейки = (getattr(self, "_склейка_для", -1) == мой_номер)
            if оборвали and сказано and not ради_склейки:
                кусок = " ".join(сказано).strip()
                if кусок:
                    # ⚠️ пишем ТОТ вопрос, на который отвечали, а не склеенный:
                    # иначе в памяти окажется пара «полная фраза → ответ на огрызок»
                    self.llm.history.append({"role": "user", "content": вопрос_на_который_отвечали})
                    # ⚠️ Метку «меня перебили» ставим ТОЛЬКО если человек правда
                    # перебил голосом. Обрыв бывает и от нового вопроса вне окна
                    # склейки — тогда никто не перебивал, и врать про это в память
                    # не надо, иначе помощник начнёт «продолжать» не то.
                    метка = (" …(здесь меня перебили — если попросят продолжить, продолжай с этого места)"
                             if перебили_голосом else " …(не договорил)")
            ...
            self._ответ_кончился = time.time()   # 🧩 для склейки: закрываем окно гонки
            self.is_processing = False
            self.is_speaking = False
