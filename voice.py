...
class EdgeTTSTurbo:
    ...
    # ⏱️ 26.08 СКОЛЬКО БАЙТ В СЕКУНДЕ ЗВУКА — у голосов РАЗНАЯ плотность,
    # замерено на живых файлах: Ермил 8000, Дмитрий 6000. Нужна, чтобы по
    # объёму отправленного узнать, сколько это будет ЗВУЧАТЬ.
    # ⚠️ Одной цифрой обойтись нельзя: ошибка вышла бы в треть, и часы
    # показывали бы, что ответ ещё идёт, когда он давно кончился.
    БАЙТ_В_СЕКУНДЕ_ЯНДЕКС: int = 8000
    БАЙТ_В_СЕКУНДЕ_EDGE: int = 6000

    _последний_движок: str = "edge"   # кто озвучивал прошлый кусок

    @classmethod
    def байт_в_секунде(cls) -> int:
        return (cls.БАЙТ_В_СЕКУНДЕ_ЯНДЕКС if cls._последний_движок == "yandex"
                else cls.БАЙТ_В_СЕКУНДЕ_EDGE)
    ...
    async def _ya_один(self, text: str) -> bytes:
        ...
        EdgeTTSTurbo._последний_движок = "yandex"
    ...
    async def synthesize(self, text: str) -> bytes:
        ...
        # 🔀 26.08 ЕРМИЛ ОСНОВНОЙ ВЕЗДЕ, ДМИТРИЙ — ПОДСТРАХОВКА.
        # Пробы прошли, и он решил: Ермил на ОБЕИХ моделях, и на быстрой,
        # и на умной. Дмитрий остаётся запасным на случай, когда Яндекс не
        # отвечает — гостям тишина недопустима.
        # ⚠️ Раньше (24.08) дорожки были разведены наглухо: на умной Яндекс
        # без подстраховки, «не работает — не работает, никто не видит».
        # Это было для проб. Теперь голос идёт всем, и подстраховка нужна.
        # Худший случай молчания: 2+2 у Яндекса, потом 3×7 у Дмитрия = 25 с.
        сырой = text
        ...
        # ⏱️ отмечаем, кто озвучивает: у голосов разная плотность звука, и по
        # ней считается, сколько ответ будет ЗВУЧАТЬ (см. байт_в_секунде).
        EdgeTTSTurbo._последний_движок = "edge"
    ...
    async def synthesize_streaming(self, text: str, send_callback) -> None:
        ...
        """
        # 🔀 26.08 ЕРМИЛ ОСНОВНОЙ ВЕЗДЕ (см. пояснение в synthesize).
        # Отдал звук — отправляем и уходим. Не отдал — НЕ молчим, как раньше,
        # а спускаемся к Дмитрию: голос теперь слышат все, тишина недопустима.
        сырой = text
        if self.ya_on:
        ...
        # ⏱️ ОТМЕЧАЕМ ДВИЖОК И ЗДЕСЬ. Найдено глазами при финальной вычитке:
        # отметка стояла только в synthesize, а живой разговор идёт ЧЕРЕЗ ЭТУ
        # функцию. Без неё после отказа Яндекса оставалась метка «yandex», и
        # длительность речи Дмитрия считалась по чужой плотности — ошибка в
        # треть, часы врали бы сильнее, чем до всей правки.
        EdgeTTSTurbo._последний_движок = "edge"
        text = _tts_clean(text, self.lang)   # словарь произношения + чистка
        if not text or not text.strip():
            return
        ...
        try:
            ...
            if self.first_message:
                ...
                if self.cached_filler_audio and not self.barge_in_requested and _я_актуален():
                    ...
                    # ⏱️ присказка тоже ЗВУЧИТ (около трёх секунд) и входит в
                    # общую длину ответа — иначе часы недосчитают, и человек,
                    # дослушав, не сможет продолжить мысль. Найдено глазами.
                    self._отослано_байт = (getattr(self, "_отослано_байт", 0)
                                           + len(self.cached_filler_audio))
            ...
            async for text_chunk in self.llm.generate_stream_turbo(
                ...
                async def send_audio(audio_bytes):
                    ...
                    if not self.barge_in_requested and _я_актуален():
                        ...
                        # ⏱️ 26.08 СЧИТАЕМ ОТПРАВЛЕННОЕ. Из объёма узнаём, сколько
                        # звук будет ЗВУЧАТЬ, — а это и есть миг, когда человек
                        # дослушает. Раньше отметка «ответ кончился» ставилась в
                        # миг отправки и на длинном ответе врала на десятки секунд:
                        # сервер шлёт 11.8 с звука за 1.76 с.
                        self._отослано_байт = getattr(self, "_отослано_байт", 0) + len(audio_bytes)
            ...
            if not оборвали and not сказано and getattr(self, "lang", "ru") != "en":
                ...
                async def send_audio_fallback(audio_bytes):
                    if not self.barge_in_requested and _я_актуален():
                        await self.websocket.send_bytes(audio_bytes)
                        # ⏱️ считаем и здесь: иначе после запасной фразы часы
                        # встанут в текущий миг, будто ответа не было вовсе,
                        # и продолжение разговора не склеится. Найдено глазами.
                        self._отослано_байт = getattr(self, "_отослано_байт", 0) + len(audio_bytes)
            ...
            if _я_актуален():
                ...
                try:
                    ...
                    pass
                # ⏱️ 26.08 ОТМЕТКА «ОТВЕТ КОНЧИЛСЯ» — КОГДА ЧЕЛОВЕК ДОСЛУШАЕТ,
                # а не когда мы дослали. Раньше стояло просто time.time(), и
                # на длинном ответе часы врали: сервер отправляет 11.8 с звука
                # за 1.76 с, а человек слушает все 11.8. Из-за этого правило
                # склейки (продолжение в пределах 3.5 с после ответа) почти
                # никогда не срабатывало на длинных ответах — продолжение
                # мысли принималось за НОВЫЙ вопрос, и нить рвалась.
                # Считаем по объёму и плотности голоса (у Ермила и Дмитрия она разная).
                # ⚠️ ЕСЛИ ПЕРЕБИЛИ — отметка ставится в ТЕКУЩИЙ миг, без
                # добавки: человек оборвал ответ, значит он кончился сейчас, а
                # не когда-то потом. Иначе часы застряли бы в будущем и
                # склеивали бы новые вопросы к оборванной мысли.
                _байт = getattr(self, "_отослано_байт", 0)
                if оборвали or self.barge_in_requested or not _байт:
                    self._ответ_кончился = time.time()
                else:
                    self._ответ_кончился = time.time() + _байт / EdgeTTSTurbo.байт_в_секунде()
                self._отослано_байт = 0
                self.is_processing = False
                self.is_speaking = False
