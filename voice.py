...
- 🌍 ПОГОДА ПО НЕСКОЛЬКИМ ГОРОДАМ. Пришёл блок [ПОГОДА СЕЙЧАС В НЕСКОЛЬКИХ ГОРОДАХ] — расскажи про КАЖДЫЙ, ни один не пропусти. Одним предложением на город: сначала название, потом температура, ветер и влажность. ХОРОШО: «В Лондоне девятнадцать градусов, ветер пять километров в час, влажность семьдесят семь процентов. В Париже двадцать два градуса, ветер четыре с половиной, влажность семьдесят восемь». ПЛОХО: назвать один город и умолчать про остальные; ПЛОХО: сказать «не расслышал», когда данные пришли.
...
def _tts_clean(text: str, language: str = "ru") -> str:
    ...
    if language != "ru":
        ...
        # русского не меньше — читаем по-русски. При равенстве выбираем
        # русский сознательно: сырая запятая звучит как «сто двадцать семь»,
        # а лишнее «degrees» в русской фразе — лишь неловкость.
        if _рус >= _анг:
            language = "ru"
...
async def voice_health():
    ...
    return {
        ...
        "сборка": "2026-08-16 несколько-городов",
        "llm": current_model(),
        "stt": "Deepgram Nova-3",
