...
ГРАМОТНОСТЬ РУССКОГО — СТРОГОЕ ПРАВИЛО:
- Пиши существующими русскими формами. Следи за падежом, родом и числом, особенно в причастиях: «пропущенные», а не «пропусканные»; «включённый», а не «включанный».
- Не изобретай слов и не собирай форму по аналогии. Не уверен в форме — возьми простое слово, в котором уверен, или перестрой фразу.
- Тебя слушают вслух: кривое слово режет ухо сильнее, чем на письме.
...
_ВРЕМЯ_БРАУЗЕРА_МС = 480   # redemptionFrames 15 × 32 мс в quantareon-voice.js


def _хвостовое_ожидание() -> float:
    """Сколько ждать хвост фразы после сигнала браузера, в секундах.

    Считаем сами: разница между секундомером распознавателя (DG_ENDPOINTING)
    и секундомером браузера (~480 мс), плюс 150 мс на дорогу. Поднимешь
    DG_ENDPOINTING — ожидание подтянется само, руками ничего не трогать.
    Переменной SPEECH_GRACE_SEC можно задать своё число, «0» — выключить.
    """
    явное = os.getenv("SPEECH_GRACE_SEC", "").strip()
    if явное:
        try:
            return max(0.0, float(явное))
        except ValueError:
            pass
    try:
        dg = float(os.getenv("DG_ENDPOINTING", "300"))
    except ValueError:
        dg = 300.0
    return max(0.0, (dg - _ВРЕМЯ_БРАУЗЕРА_МС) / 1000.0) + 0.15
...
def _мысль_не_закончена(текст: str) -> bool:
    ...
    # ❓ ВОПРОСИТЕЛЬНЫЙ ЗНАК СИЛЬНЕЕ ВСЕГО ОСТАЛЬНОГО. «Согласен?», «Понял?»
    # — это вопрос К ПОМОЩНИКУ, на него надо отвечать сразу, хотя те же слова
    # без знака («Согласен.») означали бы, что человек ещё договаривает.
    if т[-1] == "?":
        return False
    ...
    if len(слова) <= 1:
        return True
    # ⚠️ ГЛАВНЫЙ ПРИЗНАК, найденный 11.08 по его скриншотам. Распознаватель
    # закрывает ЗАКОНЧЕННУЮ фразу знаком: «не покладая рук, как говорится.
    # Согласен?». А там, где он оборвал человека на середине, знака нет вовсе:
    # «Всё нормально, дружище. Тоже», «Хорошо, будем работать совместно».
    # Значит отсутствие знака в конце — это и есть «человек ещё говорит».
    # Ждём продолжения. Цена платится ТОЛЬКО здесь: фраза, закрытая точкой
    # или вопросом, уходит в работу сразу, без единой лишней миллисекунды.
    return True


# ═══════════════════════════════════════════════════════
#  ✍️ КРИВЫЕ РУССКИЕ ФОРМЫ — ИСПРАВЛЯЛКА ТЕКСТА
# ═══════════════════════════════════════════════════════
# ⚠️ ЭТО НЕ СЛОВАРЬ УДАРЕНИЙ. Тот (_UDAR_WORDS ниже) правит только ЗВУК,
# и написанное в окне ему недоступно. Этот список правит САМ ТЕКСТ — и на
# экране, и в озвучке разом, потому что стоит до их развилки.
#
# Зачем: модель (gpt-oss, англоязычная по природе) изредка лепит несуществующую
# русскую форму. Он поймал 11.08 «пропусканные фразы» вместо «пропущенные».
# Правилом «пиши грамотно» это не чинится — только поимённо, как со словарём
# ударений в аудиокниге: слово поймал — слово вписал.
#
# КАК ПОПОЛНЯТЬ: пара (кривая ОСНОВА, верная ОСНОВА), без окончания —
# тогда правило берёт разом все падежи и роды. Регистр сохраняется сам.
# Основу бери подлиннее, чтобы не задеть законное слово.
_КРИВЫЕ_ОСНОВЫ = [
    ("пропусканн", "пропущенн"),   # пропусканные фразы → пропущенные
]


def _почини_формы(текст: str) -> str:
    """Заменить несуществующие русские формы на правильные."""
    т = текст or ""
    for кривая, верная in _КРИВЫЕ_ОСНОВЫ:
        def _r(m, верная=верная):
            слово = m.group(0)
            хвост = слово[len(кривая):]
            замена = верная + хвост
            return замена[0].upper() + замена[1:] if слово[:1].isupper() else замена
        т = re.sub(re.escape(кривая) + r"[а-яё]*", _r, т, flags=re.IGNORECASE)
    return т
...
ИНСТРУМЕНТЫ = [{
    ...
    "function": {
        ...
        "description": (
            ...
            "НЕ ВЫЗЫВАЙ НИКОГДА:\n"
            "— 🤝 СВЕТСКИЙ ВОПРОС, ОБРАЩЁННЫЙ К ТЕБЕ ЛИЧНО. «Как дела?», "
            "«что нового?», «как жизнь?», «как ты?», «что расскажешь?», «чем "
            "занят?» при встрече — это ПРИВЕТСТВИЕ, человек спрашивает про "
            "ТЕБЯ, а не просит сводку новостей. Отвечай сам, по-человечески, "
            "и никуда не ходи. Разница видна по добавке: «что нового В МИРЕ», "
            "«что нового В НОВОСТЯХ», «что нового В ГРУЗИИ» — вот это уже "
            "просьба про мир, её ищи. Голое «что нового?» после «привет» — "
            "нет. Сомневаешься — не ходи, а переспроси одним вопросом;\n"
...
class VoiceSessionTurbo:
    ...
    async def on_speech_end(self, from_browser: bool = False):
        ...
        # ⏳ ЖДЁМ ХВОСТ ФРАЗЫ — ДВА СЕКУНДОМЕРА ДОЛЖНЫ СОЙТИСЬ.
        # ⚠️ УРОК 11.08. Конец реплики отмеряют ДВОЕ, и решает тот, кто быстрее.
        # Детектор в браузере объявляет «договорил» примерно через 480 мс тишины
        # (redemptionFrames 15 по 32 мс), а распознаватель отдаёт последний
        # кусок текста только через DG_ENDPOINTING. Когда подняли распознаватель
        # до 700, браузер стал кричать «готово» РАНЬШЕ, чем приходил хвост
        # фразы, — и хвост прилетал уже отдельной репликой. Он это и увидел:
        # «Всё нормально, дружище. Тоже» и следом отдельно «более-менее хорошо».
        # Лечение: после сигнала браузера подождать ровно разницу между
        # секундомерами. Срок считается САМ от DG_ENDPOINTING — поменяешь его,
        # ожидание подстроится. Переменной SPEECH_GRACE_SEC можно задать своё
        # число секунд, 0 — выключить совсем.
        хвост = _хвостовое_ожидание()
        if from_browser and хвост > 0:
            ...
            while ждали < хвост:
                ...
                if self.transcript_buffer != было:
                    было = self.transcript_buffer
                    # хвост пришёл и фраза закрыта знаком — ждать больше нечего
                    if было.rstrip()[-1:] in ".!?…":
                        break
        ...
        try:
            ...
            async for text_chunk in self.llm.generate_stream_turbo(
                ...
                if getattr(self, "lang", "ru") != "en":
                    ...
                    if not text_chunk:
                        continue
                    # ✍️ Кривые русские формы правим ЗДЕСЬ — до того, как
                    # текст разойдётся на экран и в озвучку. Значит чинится
                    # и написанное, и произносимое разом.
                    text_chunk = _почини_формы(text_chunk)
