...
class Config:
    ...
    # 11.08: включён ПРЯМО ЗДЕСЬ, переменная на Render больше не нужна —
    # он справедливо сказал, что плодить переменные незачем, правда должна
    # лежать в одном месте, в коде. Выключить = поставить "0" в этой строке.
    TURN_JUDGE: bool = os.getenv("USE_TURN_JUDGE", "1") == "1"
    ...
    # 11.08, вторая ступень по его слову: 3.0 → 2.5 → **2.0**.
    # Обмен прямой и его надо помнить: пауза перед ответом короче, но
    # раздумье длиннее двух секунд теперь режется. Дальше вниз идти опасно —
    # у него в дневнике встречались паузы посреди мысли по 2 секунды.
    # 11.08, его слово: «пауза и так большая, убери совсем, проверим, что даст».
    # НОЛЬ = судья больше никого не задерживает: он ещё считает и пишет свой
    # вердикт в дневник, но ждать по нему сервер не будет. Значит резать станет
    # как раньше — это проба, а не решение.
    TURN_MAX_WAIT: float = float(os.getenv("TURN_MAX_WAIT", "0"))
...
class VoiceSessionTurbo:
    ...
    def _schedule_autoflush(self, через: float = 0.0):
        ...
        ⚠️ 11.08, ПРОБА ПО ЕГО СЛОВУ «всё по нулям»: срок 0.0 — выжидания нет
        вовсе, сервер решает сразу. Это КРАЙНЯЯ ТОЧКА для замера, не рабочая
        настройка: вместе с нулевым TURN_MAX_WAIT ничто больше не удерживает
        реплику, и резать будет на каждой заминке.
        ⚠️ 11.08: срок опущен с 2.8 до 1.2 секунды. Прежние 2.8 ставились,
        когда страховка была СЛЕПАЯ — она сразу отдавала реплику, поэтому и
        выжидала подольше, для безопасности. Теперь её сторожит судья: даже
        если она сработает рано, он скажет «человек ещё говорит», и ничего
        не отрежется. Значит выжидать вслепую больше незачем — это был самый
        жирный кусок паузы перед ответом."""
        try:
