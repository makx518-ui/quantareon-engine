...
Адреса:
  POST /api/oplata/zakaz    {pochta, tarif: den|shiv1|shiv7|shiv30, lang}   — открыт
...
_ПОПОЛНЕНИЕ = re.compile(r"пополнени|зачислени|поступлени|перевод от|входящий", re.I)
...
def суммы_из_текста(текст):
    ...
    текст = текст or ""
    if not _ПОПОЛНЕНИЕ.search(текст):
        return []
...
def _отчёт(з):
    имена = {"den": "разовый день", "shiv1": "книга · 1 день", "shiv7": "книга · 7 дней", "shiv30": "книга · 30 дней"}
    return (f"✅ Оплата {з['сумма']} ₽ — {имена[з['тариф']]}\n"
            f"Почта: {з['почта']}\nКлюч: {з['ключ']}\n"
            ...
            f"{('Банк: ' + з['пуш'] + chr(10)) if з.get('пуш') else ''}"
