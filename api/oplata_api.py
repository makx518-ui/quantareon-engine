...
Адреса:
  POST /api/oplata/zakaz    {pochta, tarif: den|shiv7|shiv30|nedelya|mesyac|god, lang, moment}   — открыт
...
ТАРИФЫ = {
    ...
    "shiv7":  (1500, "shiv", 7),
    "shiv30": (6000, "shiv", 30),
    # 23.09 · гороскопы на срок: оплачено → заказ сразу в работу (api/srok_api.py), файл — на почту
    "nedelya": (800,  "srok", 7),
    "mesyac":  (1600, "srok", 30),
    "god":     (3600, "srok", 365),
...
ТОВАРЫ_ФАЙЛЫ = {
    ...
    # 26.09 · английский перевод — отдельный тариф, свой файл, своё имя вложения и страница
    "kniga-telepat-en": {
        "путь": os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "telepath_en.pdf"),
        "imya": "Telepath.pdf",
        "название": "Telepath",
        "stranica": "telepat",
    },
...
async def zakaz(request: Request):
    ...
    if ТАРИФЫ[тариф][1] == "srok" and not секунда:
...
def _закрыть(з, как):
    ...
    if вид == "srok":
        # 23.09 · гороскоп на срок: ключ = номер его заказа; в работу ставит _после_оплаты (вне замка)
        import srok_api as S
        номер_срока = secrets.token_hex(8)
        зап = {"ключ": S.ключ_из_номера(номер_срока), "почта": з["почта"], "тариф": з["тариф"],
               "lang": з["lang"], "вид": "srok", "номер_срока": номер_срока, "секунда": з.pop("секунда", None)}
    elif вид == "fayl":
...
def _после_оплаты(номер, зап):
    ...
    if зап.get("вид") == "srok":
        import srok_api as S
        try:
            S.поставить_оплаченный(зап["номер_срока"], зап["тариф"], зап.get("секунда"), зап["почта"], зап["lang"])
        except Exception as e:
            сбой = f"⚠️ Гороскоп НЕ встал в работу: {type(e).__name__}: {str(e)[:150]}"
            print(f"касса: {сбой}")
        ушло = S.письмо_ключа(зап["почта"], зап["ключ"], зап["тариф"], зап["lang"])
    elif зап.get("вид") == "fayl":
