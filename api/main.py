...
_РЕНДЕР_БЕЗ_ПАРОЛЯ = ("/api/render-dnya",
                      ...
                      "/api/oplata/otmena",
                      # 20.09 · товар-файл (книга): скачивание — тоже по номеру своего заказа,
                      # и только когда заказ отмечен «оплачен» (проверка внутри ручки)
                      "/api/oplata/fayl",
...
# 23.09 · гороскоп на срок от точки входа: недельный, месячный, годовой
try:
    import srok_api as _srok
    app.include_router(_srok.роутер)
    print("🗓 Гороскоп на срок: подключён")
except Exception as _e:
    print(f"⚠️ Гороскоп на срок не подключился: {type(_e).__name__}: {_e}")
...
async def render_dnya_api(request: Request):
    ...
    try:
        ...
        # СТАРОЕ — работает как прежде: ядро, сферы, часы, резюме
        ...
        # 🆕 13.09 ДОПОЛНЕНИЕ: четыре строки по градусам углов.
        # Лунный день, знак И-Цзин и финансовые часы считает сама страница
