...
# ⚠️ 23.09: pyswisseph помнит путь к эфемеридам ОТДЕЛЬНО В КАЖДОМ ПОТОКЕ. set_ephe_path выше
# срабатывает только в потоке, где модуль импортирован. Карта дня, книга дней, чтение разбора
# и гороскопы на срок считают карту в фоновом потоке — там пути нет, и Хирон молча выпадал из
# карты («seas_18.se1 not found»). Лечим в одном месте: любой вызов библиотеки в новом потоке
# сначала ставит путь для этого потока. Расчёт в главном потоке не меняется.
def _путь_в_каждом_потоке():
    import threading
    if getattr(swe, "_путь_по_потокам", False) or not EPHE_PATH:
        return
    исходный_путь = swe.set_ephe_path
    поток = threading.local()

    def _обернуть(ф):
        def обёртка(*а, **к):
            if not getattr(поток, "путь", False):
                исходный_путь(EPHE_PATH)
                поток.путь = True
            return ф(*а, **к)
        обёртка.__name__ = getattr(ф, "__name__", "swe")
        обёртка.__doc__ = getattr(ф, "__doc__", "")
        return обёртка

    for имя in dir(swe):
        if имя.startswith("_") or имя in ("set_ephe_path", "close", "Error"):
            continue
        объект = getattr(swe, имя)
        if callable(объект) and not isinstance(объект, type):
            setattr(swe, имя, _обернуть(объект))

    def свой_путь(путь):   # кто-то сам ставит путь (muhurta) — в своём потоке он и главный
        исходный_путь(путь)
        поток.путь = True
    swe.set_ephe_path = свой_путь
    swe._путь_по_потокам = True


_путь_в_каждом_потоке()
...
def _compute_selena(julian_day: float) -> float:
    ...
    # ⚠️ 05.09.2026: опора и ход выверены по двум сверкам — ZET на дату
    # рождения (10°43'00" Тельца) и живой сайт (24°11'39" Девы на 05.09).
    # Обе сошлись до нуля секунд. Было расхождение 4 угловые минуты.
    SELENA_AT_REFERENCE = 88.0284
    SELENA_SPEED_PER_DAY = 0.1408056
