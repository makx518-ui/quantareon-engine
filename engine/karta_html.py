...
ЗНАЧОК_ЗАКАЗА = {
    ...
    # 23.09 · развёртка от точки входа на срок
    "nedelya": "🌓", "mesyac": "🌕", "polgoda": "🌗", "god": "☀️",
}
...
ЗАГОЛОВКИ = {
    ...
    # 23.09 · сроки, развёрнутые от точки входа
    "nedelya": ("НЕДЕЛЯ", "от точки входа до точки выхода"),
    "mesyac": ("МЕСЯЦ", "от точки входа до точки выхода"),
    "polgoda": ("ПОЛГОДА", "от точки входа до точки выхода"),
    "god": ("ГОД", "от точки входа до точки выхода"),
}
...
ЗНАЧКИ_РАЗДЕЛОВ = {
    # 23.09 · главы сроков от точки входа — первыми, ключи только их
    "точка входа": "🌅", "точка выхода": "🌄", "стержень": "🌟",
    "обзор года": "🗺", "сценарий года": "🎭", " месяц · ": "🌙",   # 23.09 · главы года
    "дни недели": "📅", "ход недели": "📅", "ход месяца": "📅", "ход полугодия": "📅", "ход года": "📅",
    "нерв недели": "⚡", "нерв месяца": "⚡", "нерв полугодия": "⚡", "нерв года": "⚡",
    ...
    # 23.09 · главы-недели месяца — последними, чтобы не перебить ключи выше
    "неделя": "📅",
}
...
def _значок(заголовок):
    н = заголовок.lower().strip()
    # заголовок уже начинается со значка (например «💰 Удачные часы») — второй не ставим
    if н and not н[0].isalnum() and н[0] not in "«\"'":
        return ""
