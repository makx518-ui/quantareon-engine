...
TOPICS: Dict[str, Dict] = {
    "esse": {
        ...
        "words": [
            ...
            # 24.08 второе эссе «Молодой код» — без этих слов прямой вопрос
            # «что за молодой код?» тему не находил вовсе (проверено)
            "молодой код", "молодого кода", "молодом коде", "young code",
            "второе эссе", "продолжение эссе", "новое эссе", "second essay",
            # 14.09 третье эссе «Астрофрактальная астрология»
            "астрофрактальн", "третье эссе", "третьем эссе", "секунда вопроса",
            "секунду вопроса", "third essay", "astrofractal",
            # 15.09 · вопрос «what are the four doors of one day» тему не находил —
            # помощник на быстрой модели выдумал рассвет и закат
            "четыре двери", "четырёх дверей", "спираль вместо круга", "узор без швов",
            "миг вопроса", "момент вопроса", "four doors", "second of a question",
            "moment of a question", "spiral instead of a circle", "pattern without seams",
            "who holds the whole pattern", "astrofractal astrology",
    ...
    # 22.09 · страница «Поддержка» — связь с Владом для ЛЮБОГО гостя.
    # ⚠️ Стоит ПЕРЕД laboratoriya нарочно: слова «связаться», «контакт»,
    # «почта», «телеграм» есть и там, а при равном счёте выигрывает тема,
    # объявленная раньше. Гостю нужна Поддержка, а не закрытая Лаборатория.
    # Вопрос инвестора («хочу инвестировать, как связаться») всё равно уходит
    # в laboratoriya — там добавляется «инвест», и счёт выходит выше.
    "podderzhka": {
        "file": "podderzhka.txt",
        "words": [
            "поддержк", "связаться", "связатся", "написать", "напишу",
            "вам написать", "тебе написать", "куда писать", "обратная связь",
            "как задать вопрос", "контакт",
            "почт", "телеграм", "телега", "вацап", "ватсап", "вотсап",
            "whatsapp", "whats app", "нужна помощь", "живой человек",
            "support", "get in touch", "contact", "write to you", "email",
            "e-mail", "telegram", "feedback", "reach you", "talk to a human",
        ],
    },
    "laboratoriya": {
    ...
    # 21.09 · книги «Кундалини» (700₽) и «Телепат» (500₽) — платные страницы.
    # ⚠️ «кундалини» уже занято темой esse (концепция в эссе) — сюда его
    # нарочно не кладём, чтобы не ломать эссе. Вместо этого — фразы, которые
    # выдают именно вопрос о КНИГЕ (покупка/цена/скачать), они длиннее и
    # перевешивают счёт esse, когда речь явно о покупке.
    "knigi": {
        "file": "knigi.txt",
        "words": [
            "телепат", "telepath", "kundalini",
            "огонь глубин", "fire of the depths",
            "книга кундалини", "книгу кундалини", "книге кундалини", "книги кундалини",
            "книга телепат", "книгу телепат", "книге телепат",
    ...
    # пересекаются ни с одной другой темой (проверено), коллизий нет.
    "stati": {
        "file": "stati.txt",
        "words": [
            "самадхи", "мантра", "samadhi", "mantra",
        ],
    },
}
