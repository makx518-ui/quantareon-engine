...
async def отметить(запрос: Request,
    ...
    try:
        ...
        if устройство != "робот":       # роботов не считаем вовсе
            ...
            регион = (г.get("cf-region") or "").strip()[:40]
...
function посетитель(след, дата) {
  ...
  // плитки наверху — про НЕГО, а не про срез
  document.getElementById("плитки").innerHTML = `
    <div class="плитка"><b>${свои.length}</b><span>его заходов за день</span></div>
    <div class="плитка"><b>${Object.keys(страницы).length}</b><span>разных страниц</span></div>
  ...
  const строка = (имя, значение) =>
    `<tr><td>${имя}</td><td>${экран(значение || "—")}</td></tr>`;
  const заходы = свои.map(з => `
    <tr>
      <td class="когда">${экран(з.время || "")}</td>
      <td>${экран(з.страница || "—")}</td>
      <td class="тускло">${экран(з.источник || "напрямую")}</td>
  ...
  return `
    ...
    <h2>КАРТОЧКА ПОСЕТИТЕЛЯ · ${месяц_словами(дата)}</h2>
    <p class="подсказка">${экран(след)} — его суточная метка: адрес не храним, метка меняется
      каждые сутки, поэтому карточка показывает всё за этот день</p>
    <h2>КТО ОН</h2>
    <table>
      ${строка("город", первый.город || первый.страна)}
      ${строка("регион", первый.регион)}
      ${строка("устройство", первый.устройство)}
      ${строка("браузер и система", первый.браузер)}
      ${строка("язык браузера", первый.язык)}
      ${строка("оператор", первый.оператор)}
      ${строка("пришёл с", первый.источник || "напрямую")}
      ${строка("в этот день", первый.новый ? "первый раз" : "уже заходил")}
    </table>
    ${таблица("КАКИЕ СТРАНИЦЫ СМОТРЕЛ", страницы, свои.length)}
    <h2>ЕГО ЗАХОДЫ ПО ПОРЯДКУ</h2>
    <div class="обёртка"><table class="лента">
      <tr><th>время</th><th>страница</th><th>пришёл с</th><th>браузер</th></tr>
      ${заходы}</table></div>`;
}
...
function нарисовать() {
  const d = данные;
  if (!d) return;
  // режим «один посетитель» — плитки рисует сама карточка
  if (фильтр && фильтр.след) {
    document.getElementById("под").textContent = "карточка посетителя · время московское";
