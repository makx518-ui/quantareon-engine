...
async def цифры(key: str = Query(""),
    ...
    итог["лента"] = итог["лента"][-200:][::-1]      # свежие сверху
    for поле in ("страницы", "источники", "страны", "города",
                 "операторы", "устройства", "часы"):
...
function таблица(заголовок, объект, всего) {
  ...
  const строки = пары.slice(0, 12).map(([к, в]) => {
    ...
    return `<tr><td>${к}<div class="полоса" style="width:${доля}%"></div></td><td>${в}</td></tr>`;
  }).join("");
...
function лента(записи) {
  ...
  const строки = записи.slice(0, 120).map(з => `
    <tr>
      <td class="когда">${(з.дата || "").slice(5)} ${з.время || ""}</td>
      <td>${з.город || з.страна || "—"}${з.новый ? '<span class="новый">новый</span>' : ""}</td>
      <td class="тускло">${з.оператор || "—"}</td>
      <td>${з.страница || "—"}</td>
      <td class="тускло">${з.источник || "напрямую"}</td>
      <td class="тускло">${з.устройство || ""}</td>
    </tr>`).join("");
  return `<h2>КТО ЗАХОДИЛ</h2><div class="обёртка"><table class="лента">
      <tr><th>когда</th><th>откуда</th><th>оператор</th>
...
function загрузить() {
  ...
  fetch(`/api/stats?key=${encodeURIComponent(КЛЮЧ)}&period=${срок}`)
    ...
    .then(d => {
      ...
      document.getElementById("под").textContent =
        ИМЕНА[срок] + " · время московское";
      ...
      if (d.по_дням.length > 1) {
        ...
        html += `<h2>ПО ДНЯМ</h2><div class="дни">` + d.по_дням.map(x =>
          `<div class="день" style="height:${Math.max(2, x.заходов / макс * 100)}%">
